...
import {
  ...
  SETSTARREDMAILQUICKFEEDBACKVIEWMESSAGE,
} from "../../../redux/features/mailbox/mailboxSlice";
import ComposeComponenet from "./ComposeComponenet";

const ViewMessage = ({ openEdit, setOpenEdit }) => {
  // const [open, setOpen] = useState(false);

  const { selectedMail, allMails } = useSelector((state) => state.mailbox);
            ...
            <Stack direction={"row"} spacing={2} alignItems={"center"}>
              <IconButton
                onClick={async (e) => {
                  ...
                  dispatch(SETSTARREDMAILQUICKFEEDBACKVIEWMESSAGE());
                  await handleAdminStarredMail();
                  //   setOpenEdit(false);
                }}
              >
                {selectedMail?.isUserStarred === true ? (
